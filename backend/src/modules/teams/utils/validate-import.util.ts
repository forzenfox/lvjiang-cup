...
export function validateImportData(data: ImportTeamDto[], totalRows: number): ValidationResult {
  ...
  if (data.length < MIN_TEAMS || data.length > MAX_TEAMS) {
    return {
      valid: false,
      errors: [
        new ImportErrorDto(0, '', '', 'teamCount', `战队数量必须在${MIN_TEAMS}-${MAX_TEAMS}之间`),
      ],
      teamCount: data.length,
    };
  }
  ...
  for (const team of data) {
    if (!team.name || !team.name.trim()) {
      ...
      errors.push(
        new ImportErrorDto(
          ...
          '',
          'teamName',
          `战队名称不能超过${TEAM_NAME_MAX_LENGTH}个字符`,
        ),
      );
    ...
    for (const member of team.members) {
      ...
      if (!position) {
        errors.push(
          new ImportErrorDto(row, team.name, '', getFieldDisplayName('position'), '位置不能为空'),
        );
      ...
      if (member.rating !== undefined) {
        if (
          !Number.isInteger(member.rating) ||
          member.rating < RATING_MIN ||
          member.rating > RATING_MAX
        ) {
          errors.push(
            new ImportErrorDto(
              row,
              team.name,
              positionDisplay,
              getFieldDisplayName('rating'),
              `评分必须在${RATING_MIN}-${RATING_MAX}之间`,
            ),
          );
        }
      }
        ...
        if (!championValidation.valid) {
          errors.push(
            new ImportErrorDto(
              ...
              team.name,
              positionDisplay,
              getFieldDisplayName('championPool'),
              `常用英雄名称无效: ${championValidation.invalidNames.join(', ')}`,
            ),
          );
        }
      }
      ...
      if (member.bio && member.bio.length > BIO_MAX_LENGTH) {
        errors.push(
          new ImportErrorDto(
            ...
            team.name,
            positionDisplay,
            getFieldDisplayName('bio'),
            `个人简介不能超过${BIO_MAX_LENGTH}个字符`,
          ),
        );
      }
...
export function validateSheetName(workbook: any): boolean {
  return (
    workbook.worksheets && workbook.worksheets.some((ws: any) => ws.name === '战队与队员信息导入')
  );
}
