...
describe('validateImportData', () => {
  const createMember = (position: string, isCaptain = false, rowIndex?: number): ImportMemberDto => ({
    rowIndex: rowIndex || 4,
    nickname: '测试队员',
  ...
  describe('战队成员数量校验', () => {
    ...
    it('应该拒绝没有队员的战队', () => {
      ...
      expect(result.errors.some(e => e.field === 'members' || e.message.includes('至少需要 1 名队员'))).toBe(true);
    });

    it('应该拒绝超过 5 名队员的战队', () => {
      const teams = [createTeam('超编战队', [
        createMember('TOP'),
        createMember('JUNGLE'),
        createMember('MID'),
        createMember('ADC'),
  ...
  describe('位置唯一性校验', () => {
    ...
    it('应该拒绝同一战队内位置重复', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.field === '位置' && e.message.includes('不能重复'))).toBe(true);
    });
