...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // 统计数据卡片 - 使用更精确的定位器
    this.teamCountCard = page
      .locator('div')
      .filter({ has: page.locator('text=参赛战队') })
      .filter({ has: page.locator('text=已注册战队总数') })
      .first();
    this.matchCountCard = page
      .locator('div')
      .filter({ has: page.locator('text=比赛总数') })
      .filter({ has: page.locator('text=未开始') })
      .first();
    this.streamStatusCard = page
      .locator('div')
      .filter({ has: page.locator('text=直播状态') })
      .filter({ has: page.locator('text=暂无直播') })
      .first();
    this.systemStatusCard = page
      .locator('div')
      .filter({ has: page.locator('text=系统状态') })
      .filter({ has: page.locator('text=所有服务运行正常') })
      .first();
    ...
    this.loadMockDataButton = page.locator(
      'button:has-text("加载Mock数据"), button:has-text("加载示例数据")'
    );
  ...
  async expectPageLoaded() {
    // 验证页面标题包含"管理仪表盘"
    await expect(this.page.getByRole('heading', { name: '管理仪表盘' })).toBeVisible({
      timeout: 10000,
    });
    // 验证侧边栏导航存在
  ...
  async logout() {
    ...
    const logoutButton = this.page
      .locator(
        'button:has-text("退出"), button:has-text("退出登录"), a:has-text("退出"), [role="button"]:has-text("退出")'
      )
      .first();
  ...
  async getTeamCount(): Promise<number> {
    const card = this.page
      .locator('div')
      .filter({ has: this.page.locator('text=参赛战队') })
      .filter({ has: this.page.locator('text=已注册战队总数') })
      .first();
  ...
  async getMatchStats(): Promise<{
    ...
    const card = this.page
      .locator('div')
      .filter({ has: this.page.locator('text=比赛总数') })
      .filter({ has: this.page.locator('text=未开始') })
      .first();
  ...
  async getStreamStatus(): Promise<string> {
    const card = this.page
      .locator('div')
      .filter({ has: this.page.locator('text=直播状态') })
      .filter({ has: this.page.locator('text=暂无直播') })
      .first();
  ...
  async getSystemStatus(): Promise<string> {
    const card = this.page
      .locator('div')
      .filter({ has: this.page.locator('text=系统状态') })
      .filter({ has: this.page.locator('text=所有服务运行正常') })
      .first();
    const status = await card.locator('p').nth(1).textContent({ timeout: 5000 });
    return status || '正常';
  }
