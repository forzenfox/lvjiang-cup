...
test.describe('【P1】对战数据展示 - 页面访问与加载', () => {
  ...
  test('TEST-MD-001.5: 视频链接显示与跳转 @P1', async ({ page }) => {
    ...
    await page.route('**/api/matches/*/games/*', async route => {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(gameResp) });
    });
  ...
  test('TEST-MD-001.6: 视频回顾按钮功能 @P1', async ({ page }) => {
    ...
    await page.route('**/api/matches/*/games/*', async route => {
      const gameResp = createGameResponse(1, fixture, { videoBvid: 'BV1Ab4y1X7zK', winnerTeamId: 'team-a', playerStats: [] });
  ...
  test('TEST-MD-002: 访问无数据的对战详情页面 @P1', async ({ page }) => {
    ...
    await page.route('**/api/matches/*/series', async route => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true, code: 20000,
          data: { matchId: 'empty-match', teamA: { name: 'T1', id: 't1' }, teamB: { name: 'GEN', id: 'gen' }, boFormat: 'BO5', games: [] },
        }),
      });
    });
  ...
  test('TEST-MD-003: 加载状态显示 @P1', async ({ page }) => {
    ...
    await page.route('**/api/matches/*/series', async route => {
      await page.waitForTimeout(2000);
      await route.fulfill({ status: 200, body: JSON.stringify(createSeriesResponse(fixture, [{ gameNumber: 1, winner: 'red', duration: '32:45', status: 1 }])) });
    });
