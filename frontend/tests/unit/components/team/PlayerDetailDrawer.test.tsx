...
describe('PlayerDetailDrawer 组件', () => {
  describe('抽屉渲染测试', () => {
    it('当 player 不为 null 时应该显示抽屉', () => {
      render(<PlayerDetailDrawer player={mockPlayer} onClose={vi.fn()} isMobile={false} />);
      expect(screen.getByTestId('player-drawer')).toBeInTheDocument();
    });

    it('当 player 为 null 时不应该显示抽屉', () => {
      render(<PlayerDetailDrawer player={null} onClose={vi.fn()} isMobile={false} />);
  ...
  describe('关闭按钮测试', () => {
    it('点击关闭按钮应该调用 onClose 回调', () => {
      ...
      render(<PlayerDetailDrawer player={mockPlayer} onClose={onCloseMock} isMobile={false} />);
